...
function TopMovers({ title, data, up }) {
  return (
    <div className="card col-6">
      ...
      <button className="seeall">Посмотреть все</button>
    </div>);
...
function HoldingsTable({ P }) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sort.key === "price") {av = a.price;bv = b.price;} else
    if (sort.key === "value") {av = a.value;bv = b.value;} else
    {av = a.plPct;bv = b.plPct;}
            ...
            <div className="htbl-row" key={r.tkr + r.name}>
              ...
              <div className="htbl-c right">
                <span className="htbl-price tnum">
                  ...
                  <span style={{ fontWeight: "500" }}>{fmt.price(r.price)}</span>
                </span>
              </div>
              <div className="htbl-c right">
