...
/** Вычет ИИС типа А за один год: min(взносы, 400 000) × 13% */
...
export function calcIISDeduction(depositsByYear: Record<string, number>): number {
  return Object.values(depositsByYear).reduce((sum, c) => sum + calcYearDeduction(c), 0);
}
